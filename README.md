...
*Ghi chú: Kiến trúc phía Backend được phân chia theo dạng module/domain trong thư mục `backend/src/modules` (bao gồm các module chính: `auth`, `user`, `document`).*
...
2. Tạo các tệp biến môi trường `.env` từ tệp `.env.example`:
   - `backend/.env`
   - `frontend/.env`
   *(**HOTFIX DOCKER**: Hãy đảm bảo trong `backend/.env` đã thiết lập dòng `DB_HOST=mysql` (thay vì `localhost`) để backend tìm thấy đúng vùng mạng của container. Đồng thời thiết lập `CLIENT_ORIGIN` phù hợp).*
3. Từ thư mục gốc của dự án, tiến hành thực thi câu lệnh điều phối sau:
   ```bash
   docker-compose up --build -d
   ```
   *(Gợi ý hotfix: Nếu gặp lỗi syntax với phiên bản Docker mới, hãy sử dụng lệnh `docker compose up --build -d` bỏ dấu gạch ngang).*
4. Cấu trúc mạng Docker Compose sẽ thiết lập sẵn 3 container độc lập cùng tương tác:
   - **MySQL Container**: Phơi xuất cổng `3306` (thông tin mật khẩu và dữ liệu được khởi tạo nội bộ trong file config).
   - **Backend Container**: Dịch vụ vận hành ở cổng `4000` (Truy cập API: http://localhost:4000). Hệ thống khi bật bằng Docker sẽ tự động chạy file migration.
   - **Frontend Container**: Dịch vụ vận hành ở cổng `5173` (Giao diện tương tác người dùng: http://localhost:5173).
5. Để tiến hành chấm dứt hoạt động của toàn bộ các dịch vụ trên, sử dụng lệnh:
   ```bash
   docker-compose down
   ```
...
> **Lưu ý quan trọng:** Cần tiến hành mở tệp `backend/.env` để cập nhật các tham số kết nối cơ sở dữ liệu (`DB_HOST=localhost`, `DB_PORT`, `DB_USER`, `DB_PASS`, `DB_NAME`) nhằm đồng bộ phiên làm việc MySQL cục bộ của hệ máy tính.
